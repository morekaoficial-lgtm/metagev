...
import { PeriodStatus, SelfEvaluationStatus } from '@prisma/client';
...
export class EvaluationsService {
  ...
  private daysToClose(period: { year: number; month: number }): number {
    // El auto-cierre ocurre el día 11 del mes siguiente al periodo.
    const closeDate = new Date(period.year, period.month, 11); // period.month es 1-based → mes siguiente
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    return Math.max(0, Math.ceil((closeDate.getTime() - today.getTime()) / 86400000));
  }
  ...
  async submit(employeeId: string) {
    ...
    if (!period) throw new BadRequestException('No hay un periodo activo');
